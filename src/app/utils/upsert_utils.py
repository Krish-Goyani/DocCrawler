...
class PineconeUtils:
    ...
    async def load_json_files_for_pinecone(self, directory_path: str, user_id: str) -> List[Dict[str, Any]]:
        """
        ...
        for file_name in json_files:
            ...
            try:
                ...
                self.error_repo.insert_error({"error": f"Error in processing {file_name}: {str(e)}"})
                await self.error_repo.insert_error(Error(
                    user_id=user_id,
                    error_message=f"Error in upserting: {str(e)}",
                ))
                raise JsonResponseError(status_code=500, detail=f"Error in processing {file_name}: {str(e)}")

        return pinecone_records

    async def ensure_index_exists(
        self, index_name: str, pc, user_id: str, dimension: int, metric: str = "dotproduct"
    ) -> bool:
        """
        ...
        try:
            ...
            raise JsonResponseError(status_code=500, detail=f"Error creating index: {str(e)}")
