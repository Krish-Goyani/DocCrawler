...
class UpsertService:
    ...
    async def upload_vectors(self, user_id: str):
        ...
        if not os.path.exists(embeddings_folder):
            ...
            raise JsonResponseError(status_code=404, detail=f"File not found: {embeddings_folder}")
        
        # Load JSON file for Pinecone
        vector_data = await self.pinecone_utils.load_json_files_for_pinecone(embeddings_folder, user_id)
        ...
        if not vector_data:
            ...
            raise JsonResponseError(status_code=400, detail="No data found to upsert.")

        DIMENSION = len(vector_data[0]["values"])

        # Ensure index exists
        if not await self.pinecone_utils.ensure_index_exists(self.index_name, self.client, user_id, DIMENSION):
            await self.error_repo.insert_error(Error(
                    user_id=user_id,
                    error_message="Failed to create or validate index. Exiting.",
                ))
            raise JsonResponseError(status_code=500, detail="Failed to create or validate index. Exiting.")
        ...
        for i, async_result in enumerate(async_results):
            try:
                ...
                raise JsonResponseError(status_code=500,detail=f"Error in upserting: {str(e)}")
        ...
        return {"mesage": "Upsertion completed successfully!", "upsertedCount": len(vector_data)}