...
class QueryUsecase:
    def __init__(self, pinecone_service: PineconeService = Depends(), jina_reranker_service: JinaRerankingService = Depends(), embedding_utils: EmbeddingUtils = Depends()):
        self.pinecone_service = pinecone_service
        self.reranker_service = jina_reranker_service
        self.embedding_utils = embedding_utils

    async def execute(self, query: str, filters: Dict[str, Any], alpha: float = 0.5, top_k: int = 20, top_n: int = 10, user_id: str = None):
        ...
        for key, value in filters.items():
            if isinstance(value, str):
                ...
                transformed_filters[key] = {"$in": value}  # Keep lists as they are
            elif isinstance(value, bool):
                transformed_filters[key] = {"$eq": value}  # Handle booleans correctly
            elif value is None:
                transformed_filters[key] = {"$exists": False}  # Handle None values
            else:
                transformed_filters[key] = value  # Keep other values as they are

        retrieved_vectors = await self.pinecone_service.pinecone_hybrid_query(index_host, "default", top_k, alpha, dense_vec, sparse_vec, True, transformed_filters)
        
        chunked_data = self._extract_chunked_data(retrieved_vectors.get("matches"))

        reranked_results = await self.reranker_service.rerank_documents(chunked_data, query, top_n)
